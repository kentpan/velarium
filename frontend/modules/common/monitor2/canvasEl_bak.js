...
(function () {
    ...
    Canvas.Element.prototype._createCanvasIcons = function (oIcon, context) {
        var id = this._oElement.id + '-canvas-icons';
        var oIcons = document.getElementById(id);
        if (!oIcons) {
            if (Canvas.browser.isIE) {
                var canvasEl = excanvas(document.createElement('canvas'));
            } else {
            ...
            canvasEl.id = id;
           // var oIcons = this._oElement.parentNode.insertBefore(canvasEl, this._oElement);
            var oIcons = this._oElement.parentNode.appendChild(canvasEl);
            
            oIcons.width = oIcon.width;
            oIcons.height = oIcon.height;
            oIcons.style.width = oIcon.width + 'px';
            oIcons.style.height = oIcon.height + 'px';
        }
        // this will contain the oIcons
        this._oContextIcons = oIcons.getContext('2d');
    };
    Canvas.Element.prototype.setCanvasIcons = function (oIcon, context) {
        this._createCanvasIcons(oIcon, context);
        this._oIcons = {
            element: oIcon,
            source: context
        };
    ...
    Canvas.Element.prototype.clipImage = function (oImg) {
        // this._aImages[this._aImages.length] = oImg;
        if (!this._aImages) {
            return null;
        }
    ...
    Canvas.Element.prototype.clearContaier = function (oImg) {
     ...
     */
    Canvas.Element.prototype.renderAll = function (allOnTop, flag) {
    ...
    Canvas.Element.prototype.renderIcons = function () {
        // this.beforeRenderEvent.fire();  // placeholder
        this._oContextIcons.clearRect(0, 0, parseInt(this._oConfig.width), parseInt(this._oConfig.height));
        // we render the top context
        this.drawIcons(this._oContextIcons, this._oIcons);
    };
    Canvas.Element.prototype.drawIcons = function (context, oIcons) {
        var oIcon  = oIcons.element;
        var source = oIcons.source;
        var offsetY = oIcon.height / 2;
        var offsetX = oIcon.width / 2;
    ...
    //    context.fillRect(20,20,100,80);
        context.zIndex = 999;
        context.save();
        var iconSize = oIcon.getOriginalSize();
        var originalImgSize = source.getOriginalSize();
        console.log(source, originalImgSize, oIcon, iconSize);
        var piexel = 1;
        if (originalImgSize.width / originalImgSize.height > 1) {
            piexel = (originalImgSize.height/originalImgSize.width) * 2;
        } else if (originalImgSize.width / originalImgSize.height < 1) {
            piexel = (originalImgSize.width/originalImgSize.height) * 2;
        }
        context.drawImage(oIcon._oElement, 0, 0, parseInt(oIcon.width * piexel, 10), parseInt(oIcon.height * piexel, 10));
        context.restore();
    };
    /**
     ...
     */
    Canvas.Element.prototype.drawImageElement = function (context, oImg, flag) {
    ...
    Canvas.Element.prototype.drawCorners = function (context, oImg, offsetX, offsetY) {
        ...
        var originalImgSize = oImg.getOriginalSize();
        console.log(oImg, originalImgSize);  
        context.beginPath();  
       // context.fillRect(-offsetX, -offsetY, oImg.cornersize, oImg.cornersize);
